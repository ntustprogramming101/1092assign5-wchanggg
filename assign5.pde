	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
              ...
              isHit( cabbageX[i], cabbageY[i], SOIL_SIZE, SOIL_SIZE,
...
void addTime(float seconds){	
   gameTimer += seconds *60;				// Requirement #2
}
...
String convertFramesToTimeString(int frames){	// Requirement #4
  ...
  String mm = nf(floor(frames/3600), 2);
  String ss=  nf(floor(frames%3600/60), 2);
  return (mm+":"+ss);
...
color getTimeTextColor(int frames){				// Requirement #5
    ...
    return #ffffff ;
  }else if ( frames >= 1800 && frames <= 3600){
    return #ffcc00 ;
  }else if ( frames > 600 && frames <= 1800 ){
    return #ff6600 ;  
  }else{
    return #ff0000 ;
  }
